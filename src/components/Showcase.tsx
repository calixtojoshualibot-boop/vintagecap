...
import { Search, Shield, ChevronDown, Star, Phone, Mail, MapPin, ExternalLink, ShoppingBag, Store } from 'lucide-react';
    ...
    <div className="min-h-screen bg-stone-950 text-white">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-24 text-center">
          <div className="mb-6"><NBALogo size={100}/></div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {filtered.map(cap => (
            <button
              key={cap.id}
              onClick={() => setSelected(cap)}
              className="group text-left bg-stone-900 rounded-xl border border-white/10 overflow-hidden hover:border-red-500/40 transition-all hover:shadow-md"
            >
          ...
          <div className="max-w-7xl mx-auto px-6 py-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-sm">{contact.ownerName}</p>
                  </div>
                </div>
                {contact.bio && (
              ...
              <div className="space-y-4">
                ...
                {[
                  { label: 'Facebook', value: contact.facebook, icon: '📘', color: 'hover:border-blue-500 hover:bg-blue-500/10' },
                  { label: 'Instagram', value: contact.instagram, icon: '📸', color: 'hover:border-pink-500 hover:bg-pink-500/10' },
                ].map(item => item.value ? (
                  <a
                    key={item.label}
                ...
                {contact.phone && (
                  <a
                    href={`https://m.me/${contact.facebook?.split('/').pop() || ''}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold uppercase text-sm tracking-wider transition-all shadow-lg shadow-red-600/20"
                  >
                    <ShoppingBag size={16}/> Message Seller Now
                  </a>
                )}
              </div>
            </div>
